...
use crate::api_mcp::endpoints::tokens::{
    FederationTokenCreateInput, FederationTokenRevokeInput, FederationTokensListInput,
    create_federation_token, list_federation_tokens, revoke_federation_token,
};
...
pub fn FederationTokenList() -> Element {
    let mut tokens = use_server_future(move || async move {
        list_federation_tokens(FederationTokensListInput {}).await
    })?;
    ...
    rsx! {
        ...
        TokenCreateForm {
            ...
            on_submit: move |input: TokenCreateInput| {
                spawn(async move {
                    match create_federation_token(FederationTokenCreateInput {
                        label: input.label,
                        ...
                        TokenTable {
                            ...
                            on_revoke: move |id: String| {
                                spawn(async move {
                                    ...
                                    if revoke_federation_token(FederationTokenRevokeInput { id })
                                        .await
                                        .is_ok()
                                    {
                                        tokens.restart();
