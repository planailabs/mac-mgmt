...
fn service_icon(name: &str) -> &'static str {
    match name {
        ...
        n if n.contains("open-webui") || n.contains("openwebui") => {
        ...
        n if n.contains("openclaw") || n.contains("opencode") => {
            "M6.75 7.5l3 2.25-3 2.25m4.5 0h3M3.75 4.5h16.5a1.5 1.5 0 0 1 1.5 1.5v12a1.5 1.5 0 0 1-1.5 1.5H3.75a1.5 1.5 0 0 1-1.5-1.5V6a1.5 1.5 0 0 1 1.5-1.5Z"
        }
        // Grafana / monitoring — chart
        n if n.contains("grafana") || n.contains("monitor") => {
            "M3 13.125C3 12.504 3.504 12 4.125 12h2.25c.621 0 1.125.504 1.125 1.125v6.75C7.5 20.496 6.996 21 6.375 21h-2.25A1.125 1.125 0 0 1 3 19.875v-6.75ZM9.75 8.625c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125v11.25c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 0 1-1.125-1.125V8.625ZM16.5 4.125c0-.621.504-1.125 1.125-1.125h2.25C20.496 3 21 3.504 21 4.125v15.75c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 0 1-1.125-1.125V4.125Z"
        }
        // Jupyter — beaker / science
        n if n.contains("jupyter") => {
...
pub async fn easy_access_direct(
    ...
    let tunnel_exists = row
        ...
        .map(|arr| {
            arr.iter()
                .any(|t| t.get("name").and_then(|v| v.as_str()) == Some(&tunnel))
        })
        .unwrap_or(false);
    if !tunnel_exists {
