...
async fn list_tokens(customer_id: String) -> Result<Vec<Token>, ServerFnError> {
    ...
    let tokens = sqlx::query_as::<_, Token>(
        "SELECT * FROM tokens WHERE customer_id = $1 ORDER BY created_at DESC",
    )
    .bind(uuid)
    .fetch_all(&pool)
    .await
...
async fn create_token(customer_id: String, label: String) -> Result<String, ServerFnError> {
    ...
    sqlx::query("INSERT INTO tokens (customer_id, token_hash, label) VALUES ($1, $2, $3)")
        .bind(uuid)
        .bind(&hash)
        .bind(&label)
        .execute(&pool)
...
#[component]
pub fn TokenList(customer_id: String) -> Element {
