...
use crate::web::components::chat_ui::{
    ChatMsg, ChatStreamEvent, ModelEntry, PinInfo, RunningToolInfo, reason_display,
    render_message, simple_md_to_html, state_badge,
};
    ...
    let _sse_task = use_future(move || {
        ...
        async move {
            ...
            while let Ok(val) = ev.recv::<serde_json::Value>().await {
                ...
                let Ok(evt) = serde_json::from_str::<ChatStreamEvent>(val_str) else {
                    continue;
                };
