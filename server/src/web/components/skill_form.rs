...
#[server]
async fn create_skill(slug: String, name: String, description: String) -> Result<Skill, ServerFnError> {
    let pool = crate::server_pool()?;
    let skill = sqlx::query_as::<_, Skill>(
        "INSERT INTO skills (slug, name, description) VALUES ($1, $2, $3) RETURNING *",
    )
    .bind(&slug)
    .bind(&name)
    .bind(&description)
...
pub fn SkillForm() -> Element {
    ...
    let on_submit = move |evt: FormEvent| {
        ...
        spawn(async move {
            match create_skill(slug_val, name_val, desc_val).await {
                Ok(skill) => {
                    nav.push(Route::SkillDetail { id: skill.id.to_string() });
                }
                Err(e) => {
    ...
    rsx! {
        h2 { class: "text-2xl font-bold mb-4", "New Skill" }
