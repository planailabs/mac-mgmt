...
/// Resolve skill store paths from a list of pins.
/// For each (slug, channel), looks for pin named `skill/{slug}/{channel}`.
/// Returns a map of slug → drv_full (store path).
    ...
    for (slug, channel) in skills {
        ...
        if let Some(pin) = pins.iter().find(|p| p.name == pin_name && !p.abandoned) {
            if let Some(root) = pin.roots.first() {
                result.insert(slug.clone(), root.drv_full.clone());
            }
        }
    }
