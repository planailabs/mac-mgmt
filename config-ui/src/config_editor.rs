...
use crate::config_filter_chips::FilterChips;
use crate::config_save_bar::SaveBar;
use crate::extra_config_modal::{ExtraConfigField, ExtraConfigModalHost};
...
#[component]
pub fn ConfigEditor(
    /// Opaque namespace string used for per-section localStorage keys
    /// (e.g. the cluster id on the server, "usb" on the overview).
    cluster_id: String,
    #[props(default)] read_only: bool,
    /// JSON Schema for the config (drives the form).
    ...
    if !*initialized.read() {
        let src = if initial.is_object() {
            initial.clone()
        } else {
            serde_json::json!({})
        };
        let pretty = serde_json::to_string_pretty(&src).unwrap_or_default();
    ...
    // Fire-and-forget: the consumer persists via `on_save` and reports any
    // failure through the `save_error` prop. Optimistically mark saved.
    let do_save = std::rc::Rc::new(move || {
        let text = editor_text.read().clone();
        // Wrapped in `spawn` so the outer closure stays `Fn` (signal `.set`
        // needs `&mut`); the Rc is shared by the strip + floating save buttons.
        spawn(async move {
            error.set(None);
    ...
    // The consumer owns persistence/history, so no server-side timestamp here.
    let last_saved_at: Option<String> = None;
...
fn StructuredEditor(
    ...
    let model_select_request: Signal<Option<crate::model_select_modal::ModelSelectRequest>> =
        use_signal(|| None);
    ...
    rsx! {
        ...
        crate::model_select_modal::ModelSelectModal {
                    ...
                    if let Some(ref source_kind) = model_source_kind {
                        {
                            ...
                            let mut model_select_req = use_context::<Signal<Option<crate::model_select_modal::ModelSelectRequest>>>();
                            rsx! {
                                button {
                                    r#type: "button",
                                    ...
                                    onclick: move |_| {
                                        ...
                                        model_select_req.set(Some(crate::model_select_modal::ModelSelectRequest {
