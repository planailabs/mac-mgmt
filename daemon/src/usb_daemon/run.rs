...
pub async fn run_stack(opts: UsbdOpts) -> Result<StackHandle> {
    ...
    let host_key = Arc::new(
        crate::host_keys::load_or_generate().context("load/generate SSH host key")?,
    );
    ...
    let mut svc_mgr = crate::service_mgmt::ServiceManager::init_with_services(
        &mut dcfg,
        services_vec,
...
struct LoopState {
    ...
    relay_mgr: crate::remote_ssh::RemoteSshState,
    p2p_mgr: Option<crate::p2p::P2pManager>,
...
impl LoopState {
    ...
    async fn send_heartbeat(&self) {
        ...
        let ok = crate::daemon::do_send_heartbeat(
            url,
            token,
            &self.instance_id,
            &self.host_key,
...
async fn event_loop(
    ...
    mut push_rx: Option<mpsc::Receiver<crate::server_push::PushCommand>>,
    shutdown_tx: watch::Sender<bool>,
) {
    ...
    loop {
        tokio::select! {
            ...
            _evt = async {
                ...
                if matches!(_evt, Some(crate::p2p::P2pEvent::RelayProxyUrlAcquired)) {
                    st.send_heartbeat().await;
                }
            }
        }
...
async fn build_p2p(
    ...
    let handler_state = Arc::new(crate::p2p::handler::HandlerState {
...
async fn maybe_serve_memvault(
    ...
    let peer_id = match crate::p2p::identity::keypair_from_russh(host_key) {
