//! Sovereign-AI USB *daemon*.
//!
//! `mac-mgmt usbd …` boots the reduced phone-home control plane for the
//! plan-ai-usb-minimal stack: it supervises ollama + open-webui (+ memvault)
//! **run from the mounted dmg/squashfs** (no nix install/upgrade), and adds the
//! full daemon machinery — signed heartbeat, assessment probes, relay + relay
//! SSH, and optional config-server sync of the reduced [`UsbConfig`].
//!
//! It assumes the launcher has already mounted the components and exported
//! `PLANAI_RESOURCES` (+ the resolved `PLANAI_*` paths). It does NOT prepare a
//! nix runtime (that's the separate, nix-portable `usb` build).
//!
//! Like `usb::main`, this is dispatched from `main.rs` BEFORE the tokio runtime
//! so `HOME` can be pinned single-threaded (and a future event loop can own the
